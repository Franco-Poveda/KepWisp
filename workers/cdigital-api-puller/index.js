...
amqp.connect('amqp://localhost').then(function (conn) {
    ...
    return conn.createChannel().then(function (ch) {
        ...
        ok = ok.then(function () {
            return ch.assertQueue('q0', { durable: true });
        });

        ok = ok.then(function (qok) {
            var queue = qok.queue;
            return ch.bindQueue(queue, ex, "info")
                .then(function () { return queue; });
        });

        ok = ok.then(function (queue) {
            
            return ch.consume(queue, logTransactions, { noAck: false });
        });
        return ok.then(function () {
            console.log(' [*] Waiting for logs. To exit press CTRL+C.');
        });
        function logTransactions(msg) {
                    ...
                    var qvalues = tarr.map(t => {
                        ...
                        connection.query(query, function (error, results, fields) {
                            ...
                            connection.commit(function (err) {
                                ...
                                console.log('success!');
                                ch.sendToQueue('q1',Buffer.from(JSON.stringify(qvalues)));
                                ch.ack(msg);
                            });
                        });
                    });
